...
def generate(task: SyntheticTask, output_dir: str = "data") -> str:
    ...
    # Continuous features with different distributions
    income = rng.lognormal(mean=3.5, sigma=0.8, size=n)           # right-skewed
    age = rng.normal(loc=40, scale=12, size=n).clip(18, 80)       # roughly normal
    hours_worked = rng.uniform(10, 60, size=n)                     # uniform
    distance_km = rng.exponential(scale=15, size=n)                # exponential
    ...
    # Ordinal encoded as integers (but the relationship is non-linear)
    ...
    # Non-linear interactions that reward feature engineering:
    #   - income × education interaction
    #   - age has a quadratic effect (peaks mid-career)
    #   - region shifts the baseline
    #   - hours_worked / distance_km ratio matters
    #   - satisfaction has threshold effect (>3 is a big jump)
    ...
    age_effect = -0.002 * (age - 45) ** 2 + 1.0  # peaks at 45
    ...
    if task.task_type == "classification":
        # Convert to binary using median split + noise
    ...
    # Save ground truth metadata (for verification, not given to agent)
    meta = {
        "name": task.name,
