...
from aglearn.synth_hard import HardSyntheticTask, generate_high_dim, generate_multiclass, generate_temporal_regression


def test_generate_multiclass():
    with tempfile.TemporaryDirectory() as tmp:
        task = HardSyntheticTask(name="test_mc", task_type="multiclass", metric="f1_macro", n_samples=500, seed=99)
...
def test_multiclass_imbalance():
    with tempfile.TemporaryDirectory() as tmp:
        task = HardSyntheticTask(name="test_mc_imb", task_type="multiclass", metric="f1_macro", n_samples=2000, seed=42)
...
def test_multiclass_mnar_missing():
    with tempfile.TemporaryDirectory() as tmp:
        task = HardSyntheticTask(
            name="test_mc_miss", task_type="multiclass", metric="f1_macro",
            n_samples=2000, missing_frac=0.15, seed=42,
        )
        df = pd.read_csv(generate_multiclass(task, output_dir=tmp))
...
def test_generate_temporal_regression():
    with tempfile.TemporaryDirectory() as tmp:
        task = HardSyntheticTask(name="test_temp", task_type="temporal_regression", metric="r2", n_samples=500, seed=77)
...
def test_temporal_has_outliers():
    with tempfile.TemporaryDirectory() as tmp:
        task = HardSyntheticTask(name="test_temp_out", task_type="temporal_regression", metric="r2", n_samples=2000, seed=42)
...
def test_temporal_high_cardinality():
    with tempfile.TemporaryDirectory() as tmp:
        task = HardSyntheticTask(name="test_temp_hc", task_type="temporal_regression", metric="r2", n_samples=2000, seed=42)
        df = pd.read_csv(generate_temporal_regression(task, output_dir=tmp))
...
def test_generate_high_dim():
    with tempfile.TemporaryDirectory() as tmp:
        task = HardSyntheticTask(name="test_hd", task_type="high_dim", metric="f1", n_samples=500, seed=55)
        ...
        assert len(meta["informative_features"]) == 8


def test_high_dim_feature_count():
    with tempfile.TemporaryDirectory() as tmp:
        task = HardSyntheticTask(name="test_hd_fc", task_type="high_dim", metric="f1", n_samples=200, seed=42)
...
def test_different_seeds_produce_different_hard_data():
    with tempfile.TemporaryDirectory() as tmp:
        task1 = HardSyntheticTask(name="s1", task_type="multiclass", metric="f1_macro", seed=1)
        task2 = HardSyntheticTask(name="s2", task_type="multiclass", metric="f1_macro", seed=2)
